...
@Slf4j
@Configuration
@ConditionalOnProperty(value = "task.name", havingValue = "fund-open-fund-daily-em")
public class FundOpenFundDailyEmTaskConfiguration {
    @Value("${task.name}")
    private String taskName;

    @Value("${task.fund-open-fund-daily-em.input.url}")
    private String inputUrl;

    @Value("${task.fund-open-fund-daily-em.output.path}")
    private String outputPath;

    @Value("${task.fund-open-fund-daily-em.output.file}")
